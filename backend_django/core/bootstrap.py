...
Usage (one-shot, supprimer/désactiver après utilisation) :
...
def bootstrap_superadmin(request: Request):
    ...
    if not provided or provided != settings.SECRET_KEY:
        return api_error("Clé bootstrap invalide.", status=401)

    # Sécurité: ne fonctionne que s'il n'y a AUCUN super_admin
    if User.objects.filter(role=User.ROLE_SUPER_ADMIN).exists():
        return api_error(
            "Un super_admin existe déjà. Utilisez l'interface d'administration "
            "existante pour gérer les rôles.",
            status=400,
        )
    ...
    if not email or not password or not nom or not prenom:
        return api_error("Champs requis: email, password, nom, prenom (telephone optionnel).", status=400)
    if len(password) < 8:
        return api_error("Mot de passe trop court (min 8 caractères).", status=400)
    if User.objects.filter(email=email).exists():
        return api_error("Un utilisateur avec cet email existe déjà.", status=400)
